...
export default function OrderDetailsPage() {
  ...
  const orderTimeline = [
    ...
    },
    {
      ...
      icon: (
        ...
        >
          <path
            d="M8.56131 7.44153L12.0001 1.50012L2.61643 0.255203C2.35881 0.221218 2.10906 0.358328 1.99656 0.602468L0.0658685 4.61731C-0.107569 4.98059 0.0765185 5.41028 0.442181 5.51575L7.83343 7.72395C8.13006 7.80481 8.41506 7.68762 8.56131 7.44153ZM23.9363 4.61731L22.0054 0.601687C21.8929 0.357546 21.6395 0.220515 21.3818 0.254695L12.0001 1.50012L15.4403 7.44153C15.581 7.68567 15.8671 7.80329 16.1344 7.72516L23.5557 5.51774C23.9251 5.40637 24.1051 4.97669 23.9363 4.61731ZM15.9676 9.05091C15.3301 9.05091 14.7346 8.70133 14.4113 8.14309L12.0001 4.00012L9.59256 8.14075C9.26631 8.70325 8.67381 9.05481 8.03631 9.05481C7.86756 9.05481 7.69881 9.03055 7.53943 8.98204L2.40006 7.45716V14.3361C2.40006 14.9091 2.77502 15.4044 3.30943 15.5404L11.4132 17.6427C11.7976 17.7398 12.1962 17.7398 12.5757 17.6427L20.6907 15.5404C21.1913 15.4337 21.6001 14.9376 21.6001 14.3712V7.45716L16.4626 8.97669C16.3051 9.02356 16.1363 9.05091 15.9676 9.05091Z"
            fill="#040458"
          />
        </svg>
      ),
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setTimeout(() => {
        setCurrentIndex(
          (prevIndex) => (prevIndex + 1) % trendingProducts.length,
        );
        setFade(false);
      }, 500);
    }, 5000);
  ...
  const orderItems = [
    {
      ...
      total: 200.0,
      image:
        "https://is7tai1wim.ufs.sh/f/QVO6Qx1nmSgLRVURp718rITC3AcWL9sY7gQj1du5fVO8FnEx",
    },
    {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 gap-5 bg-gray-100 p-6 md:grid-cols-2">
      ...
      <div className="w-full rounded-2xl bg-white p-6">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-2xl font-normal text-[#4F4F4F]">
            Order #VICS765
          </h2>
          <button className="rounded-lg bg-[#F5842F] px-4 py-2 text-sm text-white">
            Invoice
          </button>
        </div>

        <div className="mb-4 grid grid-cols-4 gap-4 text-sm text-[#4F4F4F]">
          <span>PRODUCT</span>
        ...
        <div className="space-y-4">
          {orderItems.map((item, index) => (
            <div key={index} className="grid grid-cols-4 items-center gap-4">
              ...
              <span className="text-center text-sm text-[#4F4F4F]">
                {item.quantity}
              </span>

              <span className="text-right text-sm text-[#4F4F4F]">
      ...
      <div className="h-fit w-full rounded-2xl bg-white p-6">
        <h2 className="mb-6 text-2xl font-normal text-[#4F4F4F]">
          Order Summary
        </h2>
        <div className="space-y-4">
        ...
        <div className="relative mt-6">
          <input
            ...
            className="w-full rounded-lg border border-[#D9D9D9] bg-[#F5F5F5] p-3 text-sm text-[#4F4F4F]"
          />
          <button
            onClick={handleApplyDiscount}
            className="absolute right-3 top-1/2 -translate-y-1/2 transform rounded-lg bg-[#F5842F] px-4 py-1 text-sm text-white"
          >
            Apply
          </button>
        </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="h-fit w-full rounded-2xl bg-white p-6">
          <h2 className="mb-4 text-lg font-normal text-[#4F4F4F]">Comment</h2>
          <div className="rounded-lg border border-[#D9D9D9] bg-[#F5F5F5] p-3">
            <textarea
        ...
        <div className="h-fit w-full rounded-2xl bg-white p-6">
          <h2 className="mb-4 text-lg font-normal text-[#4F4F4F]">
      ...
      <div className="w-full rounded-2xl bg-white p-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-normal text-[#040458]">
              Order Tracking
            </h2>
            {isEditing ? (
              <input
                ...
                className="mt-1 rounded border border-gray-300 p-1 text-sm text-[#4F4F4F]"
              />
            ) : (
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-[#D9D9D9] px-3 py-1 text-sm text-[#040458]"
          >
            <svg
              width="16"
              height="16"
        ...
        <div className="relative space-y-8">
          {orderTimeline.map((step, index) => (
            <div key={index} className="relative flex items-center gap-4">
              ...
              <div className="flex h-12 w-12 items-center justify-center rounded-lg border border-[#040458] bg-white">
                <span className="text-xl">{step.icon}</span>
              </div>

              <div>
                <p className="text-sm font-semibold text-[#040458]">
                  {step.status}
                </p>
              ...
              <div className="overflow-hidden">
                <div
                  className="absolute left-[185px] right-[22px] top-[50%] -translate-y-1/2 transform"
                  style={{
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        ...
        <div className="relative h-[150px] w-full overflow-hidden rounded-2xl shadow-lg sm:h-[180px]">
          <div
